function [vec_of_vecs, points] = MINDescriptorLL(im,kp)
area_size = 61;
nkp = size(kp,1);
...
for m = 1:nkp %nkp
    ...
    if (x > 2*area_size)  && (x < (C-2*area_size))
        if (y > 2*area_size) && (y < (L-2*area_size))
            ...
            sq = mindimage(round(y-area_size:y+area_size),round(x-area_size:x+area_size),:);
%             for n = 1:*area_size+1
%                 for k = 1:*area_size+1
%                     for p = 1:8
...
%             arr2 = rescale(arr2,0,1);
            arr1 = logsample(sq(1:32,1:32,:),1,32,16,16,3,10);
            arr2 = logsample(sq(33:68,1:32,:),1,32,16,16,3,10);
            arr3 = logsample(sq(1:32,33:68,:),1,32,16,16,3,10);
            arr4 = logsample(sq(33:68,33:68,:),1,32,16,16,3,10);
            arr = [arr1 arr3; arr2 arr4];
            arr = reshape(arr,1,[]);
            %size(arr1),size(arr4),size(arr)
